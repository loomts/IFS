...
type config struct {
	...
	n           int
	rafts       []*Raft
	applyErr    []string // from applier channel readers
	connected   []bool   // whether each server is on the net
	saved       []*Persister
	endnames    [][]string            // the port file names each sends to
	logs        []map[int]interface{} // copy of each server's committed entries
	lastApplied []int
	start       time.Time // time logAt which make_config() was called
	// begin()/end() statistics
	t0        time.Time // time logAt which test_test.go called cfg.begin()
	rpcs0     int       // rpcTotal() logAt start of test
...
// applier reads message from applier ch and checks that they match the logs
// contents
	...
	for m := range applyCh {
		if m.CommandValid == false {
			...
			if m.CommandIndex > 1 && prevok == false {
				err_msg = fmt.Sprintf("server %v applier out of order %v", i, m.CommandIndex)
			}
			if err_msg != "" {
				log.Fatalf("applier error: %v", err_msg)
	...
	for m := range applyCh {
		...
		if m.SnapshotValid {
			...
			if m.CommandIndex != cfg.lastApplied[i]+1 {
				err_msg = fmt.Sprintf("server %v applier out of order, expected index %v, got %v", i, cfg.lastApplied[i]+1, m.CommandIndex)
			}
...
// wait for logAt least n servers to commit.
	...
	for time.Since(t0).Seconds() < 10 && cfg.checkFinished() == false {
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				//DPrintf(dLog2, "nd:%v, cmd:%v, cmd1:%v", nd, cmd, cmd1)
